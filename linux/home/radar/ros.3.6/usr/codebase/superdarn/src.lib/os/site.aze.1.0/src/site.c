...
void SiteAzeExit(int signum) {
  ...
  switch(signum) {
    case 2:
      if (debug) printf("SiteAzeExit: Sig %d: %d\n",signum,AZE_exit_flag); 
      cancel_count++;
      AZE_exit_flag=signum;
      if (cancel_count < 3 )
        break;
    case 0:
      if (debug) printf("SiteAzeExit: Sig %d: %d\n",signum,AZE_exit_flag); 
...
int SiteAzeStart(char *host) {
  signal(SIGPIPE,SiteAzeExit);
  signal(SIGINT,SiteAzeExit);
  signal(SIGUSR1,SiteAzeExit);
...
/* rngoff argument in ACFCalculate.. is 2*rxchn and is normally set to 2 */
  rxchn=1;
  if (debug) {
    fprintf(stderr,"SiteAzeStart: rxchn=%d\n",rxchn);
...
int SiteAzeSetupRadar() {
  ...
  sprintf(sharedmemory,"IQBuff_Aze_%d_%d",rnum,cnum);
...
/* Setup the seqlog file here*/
  ...
  if(msglog_dir!=NULL) { 
    ...
    sprintf(msglog_name,"%s/msglog.aze%s.%04d%02d%02d",msglog_dir,channame,tstruct.tm_year+1900,tstruct.tm_mon+1,tstruct.tm_mday);
  ...
  if(seqlog_dir!=NULL) { 
    ...
    sprintf(seqlog_name,"%s/seqlog.aze%s.%04d%02d%02d",seqlog_dir,channame,tstruct.tm_year+1900,tstruct.tm_mon+1,tstruct.tm_mday);
...
int SiteAzeStartIntt(int sec,int usec) {
  ...
  SiteAzeExit(0);
  if (debug) {
    fprintf(stderr,"SiteAzeStartInt: start\n");
  }
  ...
  if (debug) {
    fprintf(stderr,"SiteAzeStartInt: end\n");
  }
  return 0;
...
*/
    ...
    TCPIPMsgRecv(sock, &rmsg, sizeof(struct ROSMsg));
    if (debug) {
      fprintf(stderr,"AZE GET_DATA:type=%c\n",rmsg.type);
      fprintf(stderr,"AZE GET_DATA:status=%d\n",rmsg.status);
    }
    smsg.type=GET_PARAMETERS;
    TCPIPMsgSend(sock, &smsg, sizeof(struct ROSMsg));
    TCPIPMsgRecv(sock, &rprm, sizeof(struct ControlPRM));
    TCPIPMsgRecv(sock, &rmsg, sizeof(struct ROSMsg));
    if (debug) {
      fprintf(stderr,"AZE GET_PARAMETERS:type=%c\n",rmsg.type);
      fprintf(stderr,"AZE GET_PARAMETERS:status=%d\n",rmsg.status);
      fprintf(stderr,"AZE Number of samples: dprm.samples:%d tsprm.samples:%d total_samples:%d\n",dprm.samples,tsgprm.samples,total_samples);
      fprintf(stderr,"AZE nave=%d\n",nave);
      fprintf(stderr,"AZE dprm.status=%d\n",dprm.status);
    }
    ...
    gmtime_r(&ttime,&tstruct);
    if(seqlog_dir!=NULL) { 
      sprintf(filename,"%s/seqlog.aze%s.%04d%02d%02d",seqlog_dir,channame,tstruct.tm_year+1900,tstruct.tm_mon+1,tstruct.tm_mday);
...
/* total_samples*8 represents number of bytes for main and back samples */
      ...
      if (debug) {
        fprintf(stderr,"AZE seq %d :: ioff: %8d\n",nave,iqoff);
        fprintf(stderr,"AZE seq %d :: rdata.main 16bit :\n",nave);
        ...
        dest = (void *)(samples);
        dest += iqoff;
        fprintf(stderr,"AZE seq %d :: rdata.back 16bit 8-11: %8d %8d %8d %8d\n",nave,
           ((int16 *)rdata.back)[8],((int16 *)rdata.back)[9],
           ((int16 *)rdata.back)[10],((int16 *)rdata.back)[11]);
        dest += dprm.samples*4;
        fprintf(stderr,"AZE seq %d :: samples    16bit 8-11: %8d %8d %8d %8d\n",nave,
           ((int16 *)dest)[8],((int16 *)dest)[9],
           ((int16 *)dest)[10],((int16 *)dest)[11]);
        fprintf(stderr,"AZE seq %d :: iqsze: %8d\n",nave,iqsze);
      }
    ...
    /* calculate ACF */   
      if (mplgexs==0) {
        ...
        fprintf(stderr,"AZE seq %d :: rngoff %d rxchn %d\n",nave,rngoff,rxchn);
        if (debug) 
          fprintf(stderr,"AZE seq %d :: ACFNormalize\n",nave);
...
*/
   ...
   free(lagtable[1]);
   if (debug) {
     fprintf(stderr,"AZE SiteIntegrate: iqsize in bytes: %ld in 16bit samples:  %ld in 32bit samples: %ld\n",(long int)iqsze,(long int)iqsze/2,(long int)iqsze/4);
     fprintf(stderr,"AZE SiteIntegrate: end: nave: %d\n",nave);
   }
