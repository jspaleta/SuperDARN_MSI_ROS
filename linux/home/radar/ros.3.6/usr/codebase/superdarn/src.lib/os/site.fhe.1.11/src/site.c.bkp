...
void SiteFheExit(int signum) {
  ...
  switch(signum) {
    case 2:
      if (debug) printf("SiteFheExit: Sig %d: %d\n",signum,FHE_exit_flag); 
      cancel_count++;
      FHE_exit_flag=signum;
      if (cancel_count < 3 )
        break;
    case 0:
      if (debug) printf("SiteFheExit: Sig %d: %d\n",signum,FHE_exit_flag); 
...
/* rngoff argument in ACFCalculate.. is 2*rxchn and is normally set to 2 */
  rxchn=1;
  if (debug) {
    fprintf(stderr,"SiteFheStart: rxchn=%d\n",rxchn);
...
int SiteFheSetupRadar() {
  ...
  sprintf(sharedmemory,"IQBuff_Fhe_%d_%d",rnum,cnum);
...
/* Setup the seqlog file here*/
  ...
  if(msglog_dir!=NULL) { 
    ...
    sprintf(msglog_name,"%s/msglog.fhe%s.%04d%02d%02d",msglog_dir,channame,tstruct.tm_year+1900,tstruct.tm_mon+1,tstruct.tm_mday);
  ...
  if(seqlog_dir!=NULL) { 
    ...
    sprintf(seqlog_name,"%s/seqlog.fhe%s.%04d%02d%02d",seqlog_dir,channame,tstruct.tm_year+1900,tstruct.tm_mon+1,tstruct.tm_mday);
...
int SiteFheStartIntt(int sec,int usec) {
  ...
  SiteFheExit(0);
  if (debug) {
    fprintf(stderr,"SiteFheStartInt: start\n");
  }
  ...
  if (debug) {
    fprintf(stderr,"SiteFheStartInt: end\n");
  }
  return 0;
...
int SiteFheIntegrate(int (*lags)[2]) {
  ...
  uint32 uQ32,uI32;
  if (debug) {
    fprintf(stderr,"FHE SiteIntegrate: start\n");
  }
  SiteFheExit(0);
...
*/
    ...
    TCPIPMsgRecv(sock, &rmsg, sizeof(struct ROSMsg));
    if (debug) {
      fprintf(stderr,"FHE GET_DATA:type=%c\n",rmsg.type);
      fprintf(stderr,"FHE GET_DATA:status=%d\n",rmsg.status);
    }
    smsg.type=GET_PARAMETERS;
    TCPIPMsgSend(sock, &smsg, sizeof(struct ROSMsg));
    TCPIPMsgRecv(sock, &rprm, sizeof(struct ControlPRM));
    TCPIPMsgRecv(sock, &rmsg, sizeof(struct ROSMsg));
    if (debug) {
      fprintf(stderr,"FHE GET_PARAMETERS:type=%c\n",rmsg.type);
      fprintf(stderr,"FHE GET_PARAMETERS:status=%d\n",rmsg.status);
      fprintf(stderr,"FHE Number of samples: dprm.samples:%d tsprm.samples:%d total_samples:%d\n",dprm.samples,tsgprm.samples,total_samples);
      fprintf(stderr,"FHE nave=%d\n",nave);
      fprintf(stderr,"FHE dprm.status=%d\n",dprm.status);
    }
    ...
    gmtime_r(&ttime,&tstruct);
    if(seqlog_dir!=NULL) { 
      sprintf(filename,"%s/seqlog.fhe%s.%04d%02d%02d",seqlog_dir,channame,tstruct.tm_year+1900,tstruct.tm_mon+1,tstruct.tm_mday);
...
/* total_samples*8 represents number of bytes for main and back samples */
      ...
      if (debug) {
        ...
        fprintf(stderr,"FHE seq %d :: iqsze: %8d\n",nave,iqsze);
      }
    ...
    /* calculate ACF */   
      if (mplgexs==0) {
        ...
        if (debug) 
        fprintf(stderr,"FHE seq %d :: ACFSumPower\n",nave);
...
*/
   ...
   free(lagtable[1]);
   if (debug) {
     fprintf(stderr,"FHE SiteIntegrate: iqsize in bytes: %ld in 16bit samples:  %ld in 32bit samples: %ld\n",(long int)iqsze,(long int)iqsze/2,(long int)iqsze/4);
     fprintf(stderr,"FHE SiteIntegrate: end: nave: %d\n",nave);
   }
